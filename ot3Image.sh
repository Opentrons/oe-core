...
# need to use docker buildkit to safely pass in secret keys
...
docker build -f "${HEREPATH}/Dockerfile" --secret id=SECRET_KEY,src=secret-file --build-arg "username=`whoami`" --build-arg "host_uid=`id -u`" --build-arg "host_gid=`id -g`" --tag "ot3-image:latest" ${tmp_dir}
...
docker run ${RUNSTYLE} --mount=type=secret,id=SECRET_KEY --rm --mount "$OE_MOUNT" --mount "$MONOREPO_MOUNT" --mount "$OT3_FW_MOUNT" -- ot3-image:latest "$@"
